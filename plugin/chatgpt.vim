...
function! GPT()
  ...
  let openaiToken = system("cat ~/.config/openai.token")
  if openaiToken isnot v:null
    let output = system("echo '" . prompt . "' | openai complete - -t $(cat ~/.config/openai.token)")
    echo "\n" . output
...
function! GPTRun()
  if confirm("Run and Ask gpt-3 about this " . &filetype . " file (Y/n)", "&Yes\n&No") == 1
    ...
    let currentFile = @%
    let output = system("(echo 'Fix " . runWith . " warnings in this " . &filetype . " code '; cat " . currentFile . "; " . runWith . " " . currentFile . ";) | openai complete - -t $(cat ~/.config/openai.token)")
    echo "\n" . output
    if confirm("Write output at cursor position? (Y/n)", "&Yes\n&No") == 1
      call feedkeys("i")
      call feedkeys(output)
